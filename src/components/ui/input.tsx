...
type InputTone = "surface" | "default";
type InputBaseProps = Omit<React.ComponentProps<"input">, "size">;

function Input({ className, type, tone, size = "md", ...props }: InputBaseProps & { tone?: InputTone; size?: InputSize }) {
  return (
    <input
      type={type}
      data-slot="input"
      className={cn(
        ...
        size === "md" && "h-9 rounded-md px-3 py-1",
